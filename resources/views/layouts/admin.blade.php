    ...
    <aside class="sidebar">
        ...
        <nav>
            ...
            @if(auth()->user()->hasPermission('site-sections.manage') || auth()->user()->hasPermission('pages.view') || auth()->user()->hasPermission('contact.manage') || auth()->user()->hasPermission('news.view') || auth()->user()->hasPermission('services.view') || auth()->user()->hasPermission('specialties.view') || auth()->user()->hasPermission('workshops.view') || auth()->user()->hasPermission('directory.view') || auth()->user()->hasPermission('documents.view') || auth()->user()->hasPermission('experiences.view') || auth()->user()->hasPermission('board.view') || auth()->user()->hasPermission('menu.view') || auth()->user()->hasPermission('events.view'))
                ...
                @if(auth()->user()->hasPermission('site-sections.manage'))<a class="{{ request()->routeIs('admin.site-sections.*') ? 'active' : '' }}" href="{{ route('admin.site-sections.index') }}"><i class="fa-solid fa-toggle-on"></i><span>Estado del sitio</span></a>@endif
                ...
                @if(auth()->user()->hasPermission('contact.manage'))<a class="{{ request()->routeIs('admin.contact.*') || request()->routeIs('admin.contact-messages.*') ? 'active' : '' }}" href="{{ route('admin.contact-messages.index') }}"><i class="fa-solid fa-inbox"></i><span>Consultas</span></a>@endif
                ...
                @if(auth()->user()->hasPermission('directory.view'))<a class="{{ request()->routeIs('admin.directory.*') ? 'active' : '' }}" href="{{ route('admin.directory.index') }}"><i class="fa-solid fa-address-book"></i><span>Directorio</span></a>@endif
                @if(auth()->user()->hasPermission('documents.view'))<a class="{{ request()->routeIs('admin.documents.*') || request()->routeIs('admin.document-categories.*') ? 'active' : '' }}" href="{{ route('admin.documents.index') }}"><i class="fa-solid fa-folder-open"></i><span>Documentos</span></a>@endif
                @if(auth()->user()->hasPermission('experiences.view'))<a class="{{ request()->routeIs('admin.experiences.*') ? 'active' : '' }}" href="{{ route('admin.experiences.index') }}"><i class="fa-solid fa-building-user"></i><span>Vinculación y práctica</span></a>@endif
                @if(auth()->user()->hasPermission('board.view'))<a class="{{ request()->routeIs('admin.board-members.*') || request()->routeIs('admin.board-records.*') ? 'active' : '' }}" href="{{ route('admin.board-records.index') }}"><i class="fa-solid fa-scale-balanced"></i><span>Junta y transparencia</span></a>@endif

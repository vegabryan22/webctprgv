...
@if($experience->documents->isNotEmpty())<section><h2>Documentos vigentes</h2><div class="document-list">@foreach($experience->documents as $document)<a href="{{ asset('storage/'.ltrim($document->file_path, '/')) }}" target="_blank" rel="noopener"><i class="fa-regular fa-file-lines"></i><span><strong>{{ $document->title }}</strong><small>{{ $document->version ? 'Versión '.$document->version.' · ' : '' }}{{ $document->responsible }}</small></span><i class="fa-solid fa-download"></i></a>@endforeach</div></section>@endif

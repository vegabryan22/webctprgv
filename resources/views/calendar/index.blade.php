...
@for($day = $calendarStart->copy(); $day->lte($calendarEnd); $day->addDay())
    ...
    @foreach($eventsByDate->get($day->format('Y-m-d'), collect()) as $event)<a class="calendar-event {{ $event->status === 'cancelled' ? 'cancelled' : '' }}" style="--event-color:{{ $event->category->color }}" href="{{ route('calendar.show', $event) }}"><strong>{{ $event->all_day ? 'Todo el día' : $event->starts_at->format('H:i') }}</strong> {{ $event->title }}</a>@endforeach
    </div>
@endfor
</div></section>

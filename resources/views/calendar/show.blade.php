...
<main class="event-detail">
    <article class="event-detail-card">
        ...
        <div class="event-detail-body">
            ...
            <span class="event-category" style="--event-color:{{ $event->category->color }}">{{ $event->category->name }}</span>
            <h1>{{ $event->title }}</h1>
            @if($event->summary)<p>{{ $event->summary }}</p>@endif
            <div class="event-meta">
                <span><i class="far fa-calendar"></i> {{ $event->starts_at->translatedFormat('l d \d\e F \d\e Y') }}@if($event->ends_at && !$event->starts_at->isSameDay($event->ends_at)) – {{ $event->ends_at->translatedFormat('l d \d\e F \d\e Y') }}@endif</span>
                <span><i class="far fa-clock"></i> {{ $event->all_day ? 'Todo el día' : $event->starts_at->format('H:i').($event->ends_at ? ' – '.$event->ends_at->format('H:i') : '') }}</span>
                @if($event->location)<span><i class="fas fa-location-dot"></i> {{ $event->location }}</span>@endif
            </div>
            <div style="margin:2rem 0">{!! $event->description !!}</div>

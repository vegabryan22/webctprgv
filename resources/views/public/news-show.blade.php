...
<main class="news-page"><article class="news-detail">
    ...
    @if($article->image_path)<img class="news-detail__image" src="{{ asset('storage/'.ltrim($article->image_path, '/')) }}" alt="">@endif
    <div class="news-detail__content">{!! $article->content !!}</div>
    @if($article->attachment_path)<a class="news-download" href="{{ asset('storage/'.ltrim($article->attachment_path, '/')) }}" target="_blank"><i class="fas fa-file-arrow-down" aria-hidden="true"></i> Descargar documento adjunto</a>@endif
</article></main>
@endsection
